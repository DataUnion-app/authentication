...
class ApiCalls {
    constructor(
        ...
        this.backendUrl = (customBackendUrl != undefined && customBackendUrl != '') ? 
            backendUrl : 'https://crab.dataunion.app/'
    }
    ...
    async getNonce(ethAddress, logErrors=false) {
        const nonceRequest = await fetch(
            `${this.backendUrl}/get-nonce?public_address=${ethAddress}`
        )
                ...
                window.web3.eth.personal.sign(
                    ...
                    (err, signed) => {
                        if (err) {
                            ...
                            return resolve({ethAddress, signed})
                        }
                    }
                ) 
            } else {
